...
DWORD CWslSNTPClient::CNtpTime::m_MsToNTP[1000] = 
{
...
struct NtpBasicInfo
{
  ...
  char m_ReferenceID[4];
	CWslSNTPClient::CNtpTimePacket m_ReferenceTimestamp;
	CWslSNTPClient::CNtpTimePacket m_OriginateTimestamp;
	CWslSNTPClient::CNtpTimePacket m_ReceiveTimestamp;
	CWslSNTPClient::CNtpTimePacket m_TransmitTimestamp;
};
#pragma pack(pop)
...
CWslSNTPClient::CNtpTime::CNtpTime()
{
  m_Time = 0;
}

CWslSNTPClient::CNtpTime::CNtpTime(const CWslSNTPClient::CNtpTime& time)
{
  *this = time;
}

CWslSNTPClient::CNtpTime::CNtpTime(CWslSNTPClient::CNtpTimePacket& packet)
{
...
CWslSNTPClient::CNtpTime::CNtpTime(const SYSTEMTIME& st)
{
...
long CWslSNTPClient::CNtpTime::GetJulianDay(WORD Year, WORD Month, WORD Day)
{
  long y = (long) Year;
  long m = (long) Month;
  long d = (long) Day;
...
void CWslSNTPClient::CNtpTime::GetGregorianDate(long JD, WORD& Year, WORD& Month, WORD& Day)
{
  long j = JD - 1721119;
  long y = (4 * j - 1) / 146097;
  j = 4 * j - 1 - 146097 * y;
...
CWslSNTPClient::CNtpTime& CWslSNTPClient::CNtpTime::operator=(const CWslSNTPClient::CNtpTime& time)
{
  m_Time = time.m_Time;
  return *this;
}

double CWslSNTPClient::CNtpTime::operator-(const CWslSNTPClient::CNtpTime& time) const
{
  if (m_Time >= time.m_Time)
	{
    CWslSNTPClient::CNtpTime diff;
...
CWslSNTPClient::CNtpTime CWslSNTPClient::CNtpTime::operator+(const double& timespan) const
{
  CWslSNTPClient::CNtpTime rVal;
	rVal.m_Time = m_Time;
...
CWslSNTPClient::CNtpTime::operator SYSTEMTIME() const
{
  //Currently this function only operates correctly in 
  //the 1900 - 2036 primary epoch defined by NTP
...
DWORD CWslSNTPClient::CNtpTime::Seconds() const
{
  return (DWORD) ((m_Time & 0xFFFFFFFF00000000) >> 32);
}

DWORD CWslSNTPClient::CNtpTime::Fraction() const
{
  return (DWORD) (m_Time & 0xFFFFFFFF);
}

CWslSNTPClient::CNtpTime::operator CWslSNTPClient::CNtpTimePacket() const
{
  CWslSNTPClient::CNtpTimePacket ntp;
  ntp.m_dwInteger = htonl(Seconds());
  ntp.m_dwFractional = htonl(Fraction());
	return ntp;
}

CWslSNTPClient::CNtpTime CWslSNTPClient::CNtpTime::GetCurrentTime()
{
  SYSTEMTIME st;
  GetSystemTime(&st);
	CWslSNTPClient::CNtpTime t(st);
  return t;
}

DWORD CWslSNTPClient::CNtpTime::MsToNtpFraction(WORD wMilliSeconds)
{
  ASSERT(wMilliSeconds < 1000);
	return m_MsToNTP[wMilliSeconds];
}

WORD CWslSNTPClient::CNtpTime::NtpFractionToMs(DWORD dwFraction)
{
	return (WORD)((((double)dwFraction) * NTP_FRACTIONAL_TO_MS) + 0.5);
}

double CWslSNTPClient::CNtpTime::NtpFractionToSecond(DWORD dwFraction)
{
...
//	USES_CONVERSION;
  ...
  ASSERT(m_hSocket != INVALID_SOCKET);
	...
	LPSTR lpszAscii = (LPTSTR)pszHostAddress;  //T2A was there
...
CWslSNTPClient::CWslSNTPClient()
{
  m_dwTimeout = 5000; //Default timeout of 5 seconds
}

BOOL CWslSNTPClient::GetServerTime(LPCTSTR pszHostName, CNtpServerResponse& response, int nPort)
{
	//For correct operation of the T2A macro, see MFC Tech Note 59
	//USES_CONVERSION;
  ...
  if (!pSocket->Create())
  {
    _RPT1(0,"Failed to create client socket, GetLastError returns: %d\n", GetLastError());
    return FALSE;
  }
  ...
  {
    _RPT3(0,"Could not connect to the SNTP server %s on port %d, GetLastError returns: %d\n", pszHostName, nPort, GetLastError());
  ...
  else
  {
    ...
    nbi.m_TransmitTimestamp = CWslSNTPClient::CNtpTime::GetCurrentTime();

    //Send off the NtpBasicInfo packet
    if (!pSocket->Send((LPCSTR) &nbi, nSendSize))
    {
      _RPT1(0,"Failed in call to send NTP request to the SNTP server, GetLastError returns %d\n", GetLastError());
    ...
    {
      _RPT1(0,"Unable to wait for NTP reply from the SNTP server, GetLastError returns %d\n", WSAETIMEDOUT);
    ...
    response.m_DestinationTime = CWslSNTPClient::CNtpTime::GetCurrentTime();
    ...
    {
      _RPT1(0,"Unable to read reply from the SNTP server, GetLastError returns %d\n", GetLastError());
    ...
    response.m_TransmitTime = nfp.m_Basic.m_TransmitTimestamp;
		...
		response.m_RoundTripDelay = (response.m_DestinationTime - response.m_OriginateTime) 
			- (response.m_ReceiveTime - response.m_TransmitTime);
    
		response.m_LocalClockOffset = ((response.m_ReceiveTime - response.m_OriginateTime) 
			+ (response.m_TransmitTime - response.m_DestinationTime)) / 2;
...
//AR 22-07-2000
#ifndef _WIN32_WCE
BOOL CWslSNTPClient::EnableSetTimePriviledge()
{
  ...
  if (!bOpenToken) 
  {
    ...
    _RPT0(0,"Failed to get Adjust priviledge token\n");
    return FALSE;
  }
  ...
  {
    _RPT0(0,"Failed in callup to lookup priviledge\n");
    return FALSE;
  }
  ...
  BOOL bSuccess = AdjustTokenPrivileges(m_hToken, FALSE, &m_TokenPriv, 0, NULL, 0);
	if (!bSuccess)
	  _RPT0(0,"Failed to adjust SetTime priviledge\n");
...
//AR 22-07-2000
#ifndef _WIN32_WCE
void CWslSNTPClient::RevertSetTimePriviledge()
{
  if (m_bTakenPriviledge) 
  {
    m_TokenPriv.Privileges[0].Attributes &= (~SE_PRIVILEGE_ENABLED);
    if (!AdjustTokenPrivileges(m_hToken, FALSE, &m_TokenPriv, 0, NULL, 0))
		  _RPT0(0,"Failed to reset SetTime priviledge\n");
    CloseHandle(m_hToken);
  }
}
#endif

BOOL CWslSNTPClient::SetClientTime(const CWslSNTPClient::CNtpTime& NewTime)
{
  BOOL bSuccess = FALSE;
//AR 22-07-2000
#ifndef _WIN32_WCE
  if (EnableSetTimePriviledge())
  {
    ...
    if (!bSuccess)
      _RPT0(0,"Failed in call to set the system time\n");
//AR 22-07-2000
#ifndef _WIN32_WCE
  }
